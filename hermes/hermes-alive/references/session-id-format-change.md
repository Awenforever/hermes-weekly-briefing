...
`capture_recent_context()` returns `{}` silently. `context_queue.json` is never refreshed. The activity guard then falls back to stale or missing context and may allow messages through.

## Detection

```bash
# Check if context_queue.json exists
ls /opt/data/hermes_alive_shared/context_queue.json
# If missing or stale, test capture directly:
...
3. Check `ls /opt/data/hermes_alive_shared/context_queue.json` — should exist and update
