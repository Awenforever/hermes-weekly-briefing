...
# Usage: bash deploy.sh [--install-deps] [--all]
# Run from the skill directory or any subdirectory.
# Assumes Hermes gateway is installed at /opt/hermes/

set -e

# Find the skill root (where SKILL.md lives)
...
deploy_hooks() {
    log "Deploying hook files from $SKILL_DIR/hooks/ to $HOOK_DIR/"
...
HERMES_PROACTIVE_WEIXIN_CHAT_ID=<your-chat-id>
...
HERMES_PROACTIVE_LLM_TIMEOUT=20
