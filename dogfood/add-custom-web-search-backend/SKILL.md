...
- **Free tier comparison (2026-06-29, verified from official pricing pages)**:
  | Backend | Free/mo | Source |
  |---------|---------|--------|
  | SerpAPI | 250 | https://serpapi.com/pricing |
  | Exa | 20,000 | https://exa.ai/pricing |
  | Brave | 1,000 ($5 credit ÷ $5/1K) | https://api-dashboard.search.brave.com/documentation/pricing |
  | Tavily | 1,000 credits | https://docs.tavily.com/documentation/api-credits |
  | Firecrawl | 500 searches (1K credits÷2cr/search) | https://www.firecrawl.dev/pricing |
  | SearchX | ∞ free | open source |
- **Quota tracking**: Hermes now tracks actual usage per backend per month in `/opt/data/web_quota.json`. After adding a backend, register its limit in the `_QUOTA_LIMITS` dict (near line 118 of web_tools.py). Quota automatically resets each calendar month. Backends with exhausted quota are skipped by `_run_search_with_fallback()`.
- **Fallback chain priority** (hardcoded in `_run_search_with_fallback()`): `exa` → `brave` → `tavily` → `searchx`. After adding a backend, decide where it should rank in the fallback chain — higher rank means tried sooner when the primary and config-fallback are both unavailable.
