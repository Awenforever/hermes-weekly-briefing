...
### 0.5 产出目录（必填）
> 周报文件和 PDF 保存在哪里？默认 `~/Work/Hermes/YYYY-MM-DD-描述性名称/`
...
### 0.7 收件人列表（必填）
> 周报发送给哪些邮箱？用逗号分隔，如 `user@example.com, colleague@example.com`
...
### 2.1 PDF 生成
使用 Typst 编译 PDF（`generate-pdf-with-cjk` v2.0.0），如不可用则 fallback 到 WeasyPrint：
```bash
cd <output-dir>
# 优先使用 Typst（推荐，CJK 支持更好）
typst compile report.typ
# 备选：weasyprint（如 typst 不可用）
python3 gen_pdf_weasy.py
```
使用 NotoSansCJK 字体，中文排版正确。
...
另一台 Hermes：
1. 安装系统依赖：Typst 或 WeasyPrint + NotoSansCJK + agently-cli
